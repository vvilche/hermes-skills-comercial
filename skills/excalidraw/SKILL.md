...
**Victor shorthand:** When Victor says "lánzate un canvas" or "haz un canvas", THIS is the tool — not a business-model-canvas HTML page. He uses "canvas" to mean Excalidraw. Always default to this skill over HTML-based alternatives for visual diagrams, architecture overviews, and proposal layouts.
...
### Business Canvas Pattern (Victor's "lánzate un canvas" use case)
See `references/business-canvas-pattern.md` for the reusable layout pattern used when Victor asks for a "canvas" — combines business model canvas blocks, architecture layers, agent/module detail, KPIs, and roadmap in one Excalidraw diagram.
